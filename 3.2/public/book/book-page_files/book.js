...
$('.btnBookID').click(function(event) {
    ...
    // } else 
    {
        fetch('http://localhost:3000/statistic', {
            ...
            body: JSON.stringify({id: Number(window.location.pathname.replace("/book/", "")), event : "click"})
          }).then((res) => {
            alert(
...
document.addEventListener("DOMContentLoaded", () => {
    fetch('http://localhost:3000/statistic', {
            ...
            body: JSON.stringify({id: Number(window.location.pathname.replace("/book/", "")), event : "visit"})
          })
})